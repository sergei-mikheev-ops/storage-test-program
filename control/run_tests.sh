...
for ((iter=1; iter<=ITERATIONS; iter++)); do
    # ✅ ИСПРАВЛЕНО: Правильное форматирование разделителя
    echo -e "\n$(printf '=%.0s' {1..60})"
    ...
    # ✅ ИСПРАВЛЕНО: Проверка количества ВМ перед запуском
    if [ ${#VMS[@]} -eq 0 ]; then
        echo "❌ Ошибка: массив ВМ пустой"
        exit 1
    fi
...
# === 15. Перезапуск для другого хранилища ===
echo "🔄 Перезапуск для другого типа хранилища..."
exec "$0"